...
class MLDSAWrapper {
  ...
  async initialize() {
    ...
    try {
      ...
      if (!this.FS.analyzePath('/working').exists) {
        this.FS.mkdir('/working');
      }
      this.FS.mount(this.module.NODEFS, { root: './nodefs' }, '/working');
  ...
  _initWrappers() {
    ...
    this._generate_csr = this.cwrap('generate_csr', 'number', ['number', 'string', 'number', 'number']);
   ...
   */
  async generateCSR(privateKey, subjectInfo, csrPath = '/working/req.csr') {
    this._ensureInitialized();
   ...
   * @param {Uint8Array} certificate - The certificate as a byte array
   * @param {Uint8Array} signature - The signature to verify
   * @param {string|Uint8Array} message - The original message
   * @param {string} [certPath='/working/cert.pem'] - Virtual FS path to save the certificate
   * @param {string} [signaturePath='/working/signature.bin'] - Virtual FS path to save the signature
   ...
   * @throws {Error} If verification process fails
   */
  async verifyWithCertificate(certificate, signature, message, certPath = '/working/cert.pem', signaturePath = '/working/signature.bin') {
    this._ensureInitialized();
    ...
    try {
      ...
      this.FS.writeFile(certPath, certificate);
      this.FS.writeFile(signaturePath, signature);
...
export default MLDSAWrapper;
// const wrapper = new MLDSAWrapper();