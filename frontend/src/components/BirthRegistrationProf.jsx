...
const MyRegistration = () => {
    ...
    const fetchRegistrations = async (page) => {
        ...
        try {
            const response = await api.get(`/birth-registration/user/${user.id}`);
            ...
            if (err.response && err.response.status === 404) {
                setError(err.response?.data?.error || 'Bạn không có thủ tục nào đang đăng ký');
                return;
            }
            ...
            <div className="flex justify-center items-center space-x-2 mt-4">
                ...
                {Array.from({ length: endPage - startPage + 1 }, (_, i) => startPage + i).map(page => (
                    <button
                        ...
                        className={`px-3 py-2 text-sm font-medium rounded-md ${pagination.currentPage === page
                                ...
                                {registrations.map((registration) => (
                                    <tr key={registration.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${registration.status === 'approved' ? 'bg-green-100 text-green-800' :
